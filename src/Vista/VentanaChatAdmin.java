 ...
 * @author juan
 */
public class VentanaChatAdmin extends javax.swing.JFrame {

    ControladorSolicitudMantenimiento controladorSolicitudMantenimiento;
    SolicitudMantenimiento soli;

    public VentanaChatAdmin(SolicitudMantenimiento soli) {
        initComponents();
        this.soli = soli;
    ...
    private void btnAtrasActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAtrasActionPerformed
        VentanaSolicitudAdmin ventana = new VentanaSolicitudAdmin();
    ...
    private void btnMensajeActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnMensajeActionPerformed
        String mensaje = "Admin: " + txtMensaje.getText();
        this.soli = controladorSolicitudMantenimiento.enviarMensajeA(this.soli, mensaje);
        actualizarMensaje();
        limpiar();
  ...
  private void actualizarMensaje() {
    ...
    for (int i = 0; i < controladorSolicitudMantenimiento.getListaAerolineas().size(); i++) {
        for (int j = 0; j < controladorSolicitudMantenimiento.getListaAerolineas().get(i).getListaSolicitud().size(); j++) {
            if (this.soli.getId().equals(controladorSolicitudMantenimiento.getListaAerolineas().get(i).getListaSolicitud().get(j).getId())) {
    ...
    for (int i = 0; i < controladorSolicitudMantenimiento.getListaAerolineas().size(); i++) {
        for (int j = 0; j < controladorSolicitudMantenimiento.getListaAerolineas().get(i).getListaSolicitud().size(); j++) {
            if (this.soli.getId().equals(controladorSolicitudMantenimiento.getListaAerolineas().get(i).getListaSolicitud().get(j).getId())) {
                ...
                for (int k = 0; k < aux.getMensajes().size(); k++) {
